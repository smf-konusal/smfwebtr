...
function bbc_ekle_code(&$codes){
		$codes[] = array(
				...
				'validate' => function(&$tag, &$data, $disabled, $params)
				{
					...
					$data = isset($disabled[$tag['tag']]) ? $url : '<img src="' . $url . '"' . $alt . $title . $params['{width}'] . $params['{height}'] . ' class="demo_resim bbc_img' . (!empty($params['{width}']) || !empty($params['{height}']) ? ' resized' : '') . '" loading="lazy">';
				},
				'disabled_content' => '($1)',
		);
		$codes[] = array(
